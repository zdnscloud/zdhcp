    ...
    const char& operator*() const {
        ...
        if (*base_ == BASE_PADDING_CHAR) {
            ...
            if (in_pad_) {
                ...
                kea_throw(BadValue, "Intermediate padding found");
            }
        } else {
            return (*base_);
        }
    ...
    vector<uint8_t>& result)
{
    ...
    while (srit != srit_end) {
        ...
        if (ch == BASE_PADDING_CHAR) {
            if (++padchars > MAX_PADDING_CHARS) {
                kea_throw(BadValue, "Too many " << algorithm
    ...
    if (padbits > BitsPerChunk * (padchars + 1)) {
        kea_throw(BadValue, "Invalid " << algorithm << " padding: " << input);
    }
    ...
    if (padbytes > 0 && *(result.end() - padbytes) != 0) {
            kea_throw(BadValue, "Non 0 bits included in " << algorithm
                      << " padding: " << input);
    }
