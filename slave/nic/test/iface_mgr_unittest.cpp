...
public:
    ...
    int getOpenSocketsCount(const Iface& iface) const {
        return iface.getSockets().size();
    }

    const kea::nic::SocketInfo*
    getSocketByAddr(const Iface::SocketCollection& sockets,
...
TEST_F(IfaceMgrTest, closeSockets) {
    std::unique_ptr<IfaceMgr> iface_mgr(new IfaceMgr());
    const Iface* iface = iface_mgr->getIface(LOOPBACK);
    ASSERT_TRUE(iface != nullptr);
    int v4_sockets_count = getOpenSocketsCount(*iface);
    ASSERT_EQ(0, v4_sockets_count);
    ASSERT_TRUE(iface->isLoopBack());
    ASSERT_TRUE(iface->isUp());
    ASSERT_TRUE(iface->isRunning());
    ASSERT_TRUE(iface->isMulticast() == false);
    ASSERT_TRUE(iface->isBroadcast() == false);
    ASSERT_TRUE(iface->isActive());
    ASSERT_EQ(iface->getPlainMac(), "00:00:00:00:00:00");

    const Iface* iface_eth0 = iface_mgr->getIface("eth0");
    ASSERT_TRUE(iface_eth0 != nullptr);
    ASSERT_TRUE(iface_eth0->isLoopBack() == false);
    ASSERT_TRUE(iface_eth0->isUp());
    ASSERT_TRUE(iface_eth0->isRunning());
    ASSERT_TRUE(iface_eth0->isMulticast());
    ASSERT_TRUE(iface_eth0->isBroadcast());
    ASSERT_TRUE(iface_eth0->isActive());
    const Iface::AddressCollection addresses = iface_eth0->getAddresses();
    ASSERT_EQ(addresses.size(), 1);
    ASSERT_EQ(addresses[0], IPAddressV4("10.0.2.15"));
    ASSERT_EQ(iface_eth0->getPlainMac(), "08:00:27:8f:dc:e2");

    ASSERT_TRUE(iface_mgr->getIface("xxx") == nullptr);
}
...
TEST_F(IfaceMgrTest, receiveTimeout6) {
    ...
    EXPECT_THROW(ifacemgr->receive6(0, 1000000), kea::BadValue);
    EXPECT_THROW(ifacemgr->receive6(1, 1000010), kea::BadValue);
}
...
TEST_F(IfaceMgrTest, receiveTimeout4) {
    ...
    // Test with invalid fractional timeout values.
    EXPECT_THROW(ifacemgr->receive6(0, 1000000), kea::BadValue);
    EXPECT_THROW(ifacemgr->receive6(2, 1000005), kea::BadValue);
}
...
// work: http://kea.kea.org/ticket/2971
#ifndef OS_BSD
...
TEST_F(IfaceMgrTest, openSockets4IfaceDown) {
    ...
    kea::dhcp::IfaceMgrErrorMsgCallback error_handler =
...
checkIfAddrs(const Iface & iface, struct ifaddrs *& ifptr) {
    ...
    if(ifptr->ifa_data != 0) {
        ...
        if ((s = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
            kea_throw(Unexpected, "Cannot create AF_INET socket");
        }

        if (ioctl(s, SIOCGIFHWADDR, & ifr) < 0) {
            close(s);
            kea_throw(Unexpected, "Cannot set SIOCGIFHWADDR flag");
        }
